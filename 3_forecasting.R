...
f_forecast_city_inhabitants <- function(cities, period){
  ...
  # If period is equal to 2040:
  if (period == "2040"){
    
    forecasted_df <- f_forecast(cities, h=21)
    names(forecasted_df) <- c(2020:2040)
    forecasted_df <- cbind(cities_and_states, data.frame(forecasted_df))
    
    # Only keep last column
    cities_forecast <- data.frame(forecasted_df[,ncol(forecasted_df)])
    ...
    cities_50k <- select(filter(cities_forecast, forecast_2040 >= 50000), c(City, forecast_2040)) 
    ...
    if (period == "2041:2060"){
      forecasted_df <- f_forecast(cities,41)
      names(forecasted_df) <- c(2020:2060)
      forecasted_df <- cbind(cities_and_states, data.frame(forecasted_df))
      
      # Only keep last 22 columns (2041-2060) (20 years + City and State)
      cities_forecast_last20 <- data.frame(forecasted_df[,(ncol(forecasted_df)-20):ncol(forecasted_df)])
      cities_forecast_last20 <- cbind(cities_and_states, cities_forecast_last20)
      
      # Check which columns have more than 50k inhabitants
      cities_50k <- cities_forecast_last20[cities_forecast_last20[,'X2040']>50000,]
      # Which have no 50k inhabitants:
      #cities_forecast_last20[cities_forecast_last20[,'X2040']<50000,][,c('City','State')]
      
      # Remove data from 2040
      cities_50k$X2040 <- NULL 
      
      # Take average from 2041 till 2060:
      avg_inhabitants <- cities_50k[,c('City','State')]
      avg_inhabitants$Means <- rowMeans(cities_50k[,-c(1:2)])
      
      cities_50k <- avg_inhabitants
...
# Execute predefined forecast function
forecasts_cities_inhabitants <- f_forecast_city_inhabitants(cities, period="2040")
#average_cities_inhabitants <- f_forecast_city_inhabitants(cities, period="2041:2060")

# Inspect final dataset
str(cities_cleaned)

# Write to csv
write.csv(cities_cleaned, file = "cities_2040.csv", row.names = TRUE)
write.csv(average_cities_inhabitants, file = "avg_inhabitants_cities.csv", row.names = TRUE)
